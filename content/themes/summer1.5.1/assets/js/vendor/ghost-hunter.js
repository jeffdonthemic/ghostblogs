...
// modified - for Summer theme purposes
...
(function( $ ) {
    ...
    var pluginMethods   = {
        ...
        find            : function( value ){
            ...
            var resultsData     = [];
            results.empty();
            // change the context for jquery reference
            var that = this;

            $.ajax({
              type: 'GET',
              url: "http://ghost-search.herokuapp.com/search?q=" + value,
              success: function(data) {
                // parse the results
                var searchResult = JSON.parse(data);
                // display the number of results found
                if(that.zeroResultsInfo) {
                  if(that.displaySearchInfo) results.append(that.format(that.info_template,{"amount":searchResult.length}));
                }

                // process the returned results and send to ghosthunter for display
                for (var i = 0; i < searchResult.length; i++) {
                  var postData = {
                    id: searchResult[i]._source.id,
                    title: searchResult[i]._source.title,
                    description: searchResult[i]._source.html,
                    link: searchResult[i]._source.url,
                    pubdate: searchResult[i]._source.published_at
                  }
                  results.append(that.format(that.result_template,postData));
                  resultsData.push(postData);
                }

              },
              error: function(err) {
                results.append(that.format(that.info_template,{"amount":"Error processing search!!"}));
              }
            });
